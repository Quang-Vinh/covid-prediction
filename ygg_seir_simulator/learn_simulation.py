...
def load_best_params_from_file(best_params_dir, country, region=None, subregion=None):
    ...
    assert os.path.isdir(
        best_params_dir
    ), f"best_params directory does not exist: {best_params_dir}"
    assert (
        country
    ), "Need to specify country to load params from file (region/subregion optional)"
    ...
    if subregion_:
        if country_ == "US":
            ...
            best_params_fname_search = (
                f"{best_params_dir}/subregion/*{country_}_{region_}_{subregion_}.json"
            )
        else:
            best_params_fname_search = (
                f"{best_params_dir}/subregion/*{country_}_{subregion_}.json"
            )
    elif country_ != "US":
...
def load_best_params_province(best_params_dir: str, province: str) -> Tuple[dict, dict]:
    """
    Load best parameters for given province in Canada

    Args:
        best_params_dir: Directory of best params
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Script to run simulations using the YYG/C19Pro SEIR model. Example: "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the calculation of the number of occupied hospital beds."
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "percentage of people we put in quarantine (e.g. 0.5 = 50%% quarantine) (default is 0)."
    ...
    parser.add_argument(
        ...
        default=-1,
        help=(
            "if --quarantine_perc is set, this is the percent reduction in transmission after quarantine."
            "For example, 0.5 means a 50%% reduction in transmission. Valid values: 0.025, 0.1, 0.25, 0.5."
        ),
    )
    parser.add_argument("--save_csv_fname", help="output csv file to save data")
    ...
    parser.add_argument(
        ...
        nargs=2,
        help=(
            "Takes two inputs, the name of the parameter and the amount to increase/decrease"
        ),
    )
    parser.add_argument(
        "--country",
