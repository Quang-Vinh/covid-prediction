...
def read_file(filename, column_id, window_size, Add_EN=True):
    ...
    if Add_EN:
        ...
        dataset = dataset[:num_data].reshape(-1,window_size)[:,window_size-1].squeeze()
...
filename_y = 'Covid19Canada-master/timeseries_canada/cases_timeseries_canada.csv'
column_id_y = 2

filename_d = 'Covid19Canada-master/timeseries_canada/mortality_timeseries_canada.csv'
column_id_d = 2

filename_r = 'Covid19Canada-master/timeseries_canada/recovered_timeseries_canada.csv'
column_id_r = 2

filename_I_C = 'Covid19Canada-master/timeseries_canada/active_timeseries_canada.csv'
column_id_I = 5
column_id_C = 2
...
r_d = r+d      #considering a simpler setting: the recovered and the death cases are treated as one compartment
...
data_train = np.concatenate((np.log(np.expand_dims(I_train, axis=1)), np.expand_dims(Z_train, axis=1)), axis=1)
...
def Predict(model_y, model_rd, I, Z, sample_EN=False):
    ...
    y_interval = model_y.confidence_intervals(np.concatenate((log_I,Z), axis=1), width=.95)
...
def Predict_Nextday(model_y, model_rd, I, Z):
    ...
    XX = np.concatenate((np.log(np.expand_dims(I, axis=1)), np.expand_dims(Z, axis=1)), axis=1)
    y = model_y.predict(XX)
    y_interval = model_y.confidence_intervals(XX, width=.95)
    
    return y, y_interval


predict_I, predict_y, interval_y = Predict_multiDay(model_y, model_rd, I[start], C[start], num_data-start, False)
#
...
Nextday_y, Nextday_interval = Predict_Nextday(model_y, model_rd, I[start-1:num_data-1], Z[start-1:num_data-1])


######################################################################
#predict using multiple samples
pred_I, pred_y = Predict_sampling(model_y, model_rd, I[start-1], C[start-1], num_data-start)
...
plt.figure()
plt.plot(x, Nextday_y, color='red', linewidth=2.0, linestyle='-.', label='predict_y_oneday')
